...
import type { MaterialItem, MaterialStatus } from '@/types/database'

type Lang = 'ar' | 'en'

// Overall status comes from the materials workflow stage
const OVERALL_AR: Record<string, string> = {
  pending: 'قيد الانتظار', preparing: 'قيد التجهيز', ready: 'جاهزة للتوريد', delivered: 'مكتملة — تم الاستلام', partial: 'استلام جزئي',
}
const OVERALL_EN: Record<string, string> = {
  pending: 'Pending', preparing: 'In Preparation', ready: 'Ready for Delivery', delivered: 'Completed — Received', partial: 'Partially Received',
}
...
export function MaterialsStatusView({
  items, projectName, clientName, overallStatus,
}: {
  items: MaterialItem[]
  projectName: string
  clientName?: string
  overallStatus: MaterialStatus | null
}) {
  const [lang, setLang] = useState<Lang>('ar')
  const t = T[lang]
  const isEn = lang === 'en'

  const done = overallStatus === 'delivered'
  const overallLabel = overallStatus
    ? (isEn ? (OVERALL_EN[overallStatus] ?? overallStatus) : (OVERALL_AR[overallStatus] ?? overallStatus))
    : (isEn ? 'Not started' : 'لم تبدأ')
  ...
  function printOut() {
    ...
    const html = `<!DOCTYPE html><html dir="${isEn ? 'ltr' : 'rtl'}" lang="${lang}"><head><meta charset="utf-8">
      ...
      <style>
        ...
        .meta b{color:#111}
        .pill{display:inline-block;padding:3px 10px;border-radius:99px;font-size:12px;font-weight:700;background:${done ? '#d1fae5' : '#fef3c7'};color:${done ? '#047857' : '#b45309'}}
    ...
    <div dir={isEn ? 'ltr' : 'rtl'} className="space-y-4">
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className={cn('rounded-full px-3 py-1 text-xs font-bold', done ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700')}>
            {overallLabel}
          </span>
          <span className="ms-auto text-sm text-gray-500">{t.total}: <strong className="text-gray-800">{items.length}</strong></span>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 text-gray-500 text-xs">
                ...
                <th className="px-3 py-2.5 text-start font-semibold">{t.doc}</th>
                <th className="px-3 py-2.5 text-start font-semibold">{t.mat}</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {items.length === 0 ? (
                ...
                <tr key={i} className="hover:bg-gray-50/50">
                  ...
                  <td className="px-3 py-2.5 text-gray-600" dir="ltr">{it.sto_no || '—'}</td>
                  ...
                  <td className="px-3 py-2.5 text-gray-600" dir="ltr">{it.unit || '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {isEn && (
        <p className="text-xs text-gray-400">
          Note: item descriptions are shown as entered by the team.
        </p>
      )}
    </div>
  )
