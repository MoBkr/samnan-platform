...
const TableHead = React.forwardRef<HTMLTableCellElement, React.ThHTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <th
      ref={ref}
      className={cn(
        'h-11 px-4 text-start text-xs font-semibold uppercase tracking-wide text-gray-500',
        className
      )}
      {...props}
    />
...
const TableCell = React.forwardRef<HTMLTableCellElement, React.TdHTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <td ref={ref} className={cn('px-4 py-3 text-gray-700', className)} {...props} />
  )
)
...
const TableEmpty = ({ message }: { message: string }) => (
  <tr>
    <td colSpan={100} className="py-12 text-center text-sm text-gray-400">
      {message}
    </td>
  </tr>
)
