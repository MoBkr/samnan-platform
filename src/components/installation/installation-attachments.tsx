...
// The installation team may VIEW the project's files: everything the
// coordinator uploads (site plans, notes, problems, materials paperwork) plus
// the contract — they need it on site. Only money stays hidden: payment
// receipts and invoices, the installation role never sees the project's cash.
const HIDDEN_TYPES = new Set(['receipt', 'invoice'])
...
export function InstallationAttachments({
  attachments, contractUrl, projectName,
}: {
  ...
  const total = visible.length + (contractUrl ? 1 : 0)
  ...
  return (
    <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
      <div className="mb-3 flex items-center gap-2">
        ...
        {total > 0 && (
          <span className="rounded-full bg-gray-100 px-2 py-0.5 text-[11px] font-bold text-gray-600">{total}</span>
        )}
      ...
      {total === 0 ? (
        ...
        <div className="space-y-2">
          {/* Contract first — the team needs it on site */}
          {contractUrl && (
            <div className="flex items-center gap-3 rounded-xl border border-brand-100 bg-brand-50/50 p-3">
              <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-white text-brand-600 shrink-0 border border-brand-100">
                {icon(contractUrl)}
              </div>
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-medium text-gray-900">
                  العقد{projectName ? ` — ${projectName}` : ''}
                </p>
                <p className="text-xs text-gray-500">
                  <span className="rounded bg-brand-100 px-1.5 py-0.5 text-[10px] text-brand-700">عقد</span>
                </p>
              </div>
